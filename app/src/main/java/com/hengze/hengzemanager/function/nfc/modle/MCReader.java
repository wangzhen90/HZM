...
package com.hengze.hengzemanager.function.nfc.modle;
...
public class MCReader {
    ...
    /**
     * 读某一块的内容
     * @param sectorIndex 块所属的扇区
     * @param blockIndex  块在所属的扇区的index，0,1,2,3
     * @param key  密码
     * @param useAsKeyB  是否使用密码B读取
     * @return
     * @throws TagLostException
     */
    public String readBlock(int sectorIndex,int blockIndex, byte[] key,
        boolean useAsKeyB) throws TagLostException {

        if(sectorIndex > 15){
            ToastUtils.showToast("扇区数不能超过15");
            return null;
        }

        if(sectorIndex > 3){
            ToastUtils.showToast("块数不能超过3");
            return null;
        }
        ...
        if (auth) {
            ...
            int actIndex = firstBlock + blockIndex;

                try {
                    byte blockBytes[] = mMFC.readBlock(actIndex);
                    // mMFC.readBlock(i) must return 16 bytes or throw an error.
                    // At least this is what the documentation says.
                    // On Samsung's Galaxy S5 and Sony's Xperia Z2 however, it
                    // sometimes returns < 16 bytes for unknown reasons.
                    ...
                    ret = Common.byte2HexString(blockBytes);
                } catch (TagLostException e) {
                    throw e;
                } catch (IOException e) {
                    // Could not read block.
            ...
            // Merge key in last block (sector trailer).
            if (!useAsKeyB) {
                if (isKeyBReadable(Common.hexStringToByteArray(ret.substring(12, 20)))) {
                    ret = Common.byte2HexString(key)
                        + ret.substring(12, 32);
                } else {
                    ret = Common.byte2HexString(key)
                        + ret.substring(12, 20) + NO_KEY;
                }
            } else {
                if (ret.equals(NO_DATA)) {
                    // If Key B may be read in the corresponding Sector Trailer,
                    // it cannot serve for authentication (according to NXP).
                    // What they mean is that you can authenticate successfully,
                    // but can not read data. In this case the
                    ...
                    //ret = NO_KEY + ret.substring(12, 20)
                    //    + Common.byte2HexString(key);
                }
            }
        }else {
            ToastUtils.showToast("卡密码验证出错！");
        }
        return ret;
    }
